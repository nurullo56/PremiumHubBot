...
✅ To'liq versiya - FIXED floating point precision
"""
import logging
...
REFERRAL_BONUS_DECIMAL = Decimal(str(BONUS_AMOUNT))  # Use Decimal for precision
...
async def get_total_referral_bonus_earned(user_id: int) -> float:
    ...
    try:
        ...
        total = Decimal(str(count)) * REFERRAL_BONUS_DECIMAL
        return float(total)  # Convert to float with proper precision
    except Exception as e:
        ...
        async with get_db() as db:
            cursor = await db.execute("""
                SELECT 
                    ...
                    CAST(COUNT(r.user_id) * ? AS REAL) as total_bonus
                FROM users u
                ...
                LIMIT ?
            """, (float(REFERRAL_BONUS_DECIMAL), limit))
